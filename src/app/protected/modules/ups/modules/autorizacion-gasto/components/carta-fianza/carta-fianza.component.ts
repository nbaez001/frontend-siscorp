...
@Component({
  selector: 'app-carta-fianza',
  templateUrl: './carta-fianza.component.html',
  styleUrls: ['./carta-fianza.component.scss']
})
export class CartaFianzaComponent implements OnInit {
  ...
  idUsuarioCoordinador: number;
    ...
    cargarDataEstado() {
      this.itemComboService.ObtenerEstadoCartaFianza().subscribe(dataItem => {
    ...
    public cargarPerfilPrefactibilidad() : void{
      ...
      this.proyectoEjecucionService.listarCartaFianza(this.pagina,this.cantidad,null)
      .subscribe(
        (wsResponseProyecto : WsResponseProyecto)=> {
    ...
    generarCabeceraColumnasEncargado(): void{
      this.columnas = [
        'Nro','codigo','nroContrato','descripcion','fecInicioEmision',
        'fecFinEmision','contratista','estado','acciones']; 
    }
  ...
  ejecutar(idProyecto: number): void{
    
    const dialogReg: MatDialogRef<EjecutarComponent> = this.dialog.open(EjecutarComponent, {
      panelClass: 'dialog-no-padding',
      width: '1000px',
      data: {
        idProyecto
  ...
  renovar(idProyecto: number): void{
    
    const dialogReg: MatDialogRef<RenovarComponent> = this.dialog.open(RenovarComponent, {
      panelClass: 'dialog-no-padding',
      width: '1000px',
      data: {
        idProyecto
