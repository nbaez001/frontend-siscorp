...
export class ModalRequerimientoComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.dataShared.obtenerIdAutoGastoGenerado.subscribe(response => {
      if (response === 0) {
        this.cargarListaRequerimiento();
      }
    });
  ...
  public cargarListaRequerimiento(): void {
    ...
    this.isLoading = true;
    this.cotizacionService.listarBandejaAutorizacionGasto(this.datos.idProyecto, this.pagina, this.cantidad, this.filtrosRequest).subscribe((wsResponseRequerimiento: any) => {
  ...
  editarRequerimiento(idCodigo: number) {
    const dialogReg: MatDialogRef<GenerarRequerimientoComponent> = this.dialog.open(GenerarRequerimientoComponent, {
      width: '1600px',
      disableClose: true,
      autoFocus: false,
  ...
  reporteDescuentoSupervisorAutoGasto() {
    this.cotizacionService.generaReporteDescuentoSupervisorAutoGasto().subscribe(response => {
      const dialogReg: MatDialogRef<PdfViewerComponent> = this.dialog.open(PdfViewerComponent, {
        disableClose: true,
        width: '90%',
        data: { titulo: '', dataBlob: response }
  ...
  reporteCuadroControlAutoGasto() {
    this.cotizacionService.reporteCuadroControlAutoGasto().subscribe(response => {
      const dialogReg: MatDialogRef<PdfViewerComponent> = this.dialog.open(PdfViewerComponent, {
        disableClose: true,
        width: '90%',
        data: { titulo: '', dataBlob: response }
  ...
  // reporteJasperToWord() {
  //   this.cotizacionService.reporteJasperToWord();
  //   this.cotizacionService.reporteCuadroControlAutoGasto().subscribe(response => {
  //     const dialogReg: MatDialogRef<PdfViewerComponent> = this.dialog.open(PdfViewerComponent, {
  //       disableClose: true,
  //       width: '90%',
