...
export class EjecutarComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder,public dialogRef: MatDialogRef<EjecutarComponent>,
...
public visualizarDatosProyecto(): void{
  ...
  ejecutarCarta(){
      ...
      this.openDialogMensajeConfirm(MENSAJES.CARTAFIANZA.EJECUTAR_CARTA, true);

      this.dialogRefMessage.afterClosed()
        .pipe(filter(verdadero => !!verdadero))
        .subscribe(() => {
          this.snackBar.open(MENSAJES.CARTAFIANZA.INFO_SUCCESS_CARTA_EJECUTAR);
